...
namespace NWG
{
	...
	// --core_methods
	void VertexBuf::OnDraw() { glBindBuffer(GL_ARRAY_BUFFER, m_unRId); }
...
namespace NWG
{
	...
	// --core_methods
	void VertexBuf::Bind() { m_pGfx->GetContext()->IASetVertexBuffers(0, 1, &m_pNative, &m_Info.szStride, &m_Info.szOffset); }
	bool VertexBuf::Remake(const GfxBufInfo& rInfo) {
		m_Info = rInfo;
