#ifndef NWG_LOAD_WINDOW_API_H
#define NWG_LOAD_WINDOW_API_H
#include <nwg_core.hpp>
#if (defined NW_GAPI)
namespace NW
{
	extern bit gfx_load_wapi();
	extern bit gfx_ext_load_wapi();
}
...
namespace NW
{
	...
	typedef v1si           (__stdcall* pfn_gfx_del_context)    (context_handle context);
	typedef context_handle (__stdcall* pfn_gfx_get_context)    (void);
	typedef v1si           (__stdcall* pfn_gfx_set_context)    (device_handle device, context_handle context);
	// device
	...
	typedef v1si           (__stdcall* pfn_gfx_ext_get_vsync)  (void);
	typedef void           (__stdcall* pfn_gfx_ext_set_vsync)  (v1si swap_interval);
}
namespace NW
{
	// context
