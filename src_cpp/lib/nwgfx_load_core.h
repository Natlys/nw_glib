#ifndef NWGFX_LOAD_CORE_H
#define NWGFX_LOAD_CORE_H
#include "nwgfx_core.hpp"
#if (defined NW_GAPI)
namespace NWGFX
{
	extern NW_API v1b gfx_load_core();
	extern NW_API void gfx_clear_err();
	extern NW_API v1b gfx_get_err_log(cstr info, cstr location, v1u line);
#	if (defined NW_DEBUG)
...
namespace NWGFX
{
	typedef HRESULT(APIENTRYP pfn_d3d_new_device_context)
	...
	typedef HRESULT(APIENTRYP pfn_d3d_new_device_context_swap)
		(_In_opt_ IDXGIAdapter*,
		D3D_DRIVER_TYPE, HMODULE, UINT,
		_In_reads_opt_(FeatureLevels) CONST D3D_FEATURE_LEVEL*,
