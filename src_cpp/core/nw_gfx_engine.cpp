...
namespace NW
{
		...
		m_prim(convert<primitives, prim>(PRIM_TRIANGLES))
	{
		...
		if constexpr (true) {
			...
			if constexpr (true) {
				...
				if (!sbuf->remake<buf_m4fm4fm4f>(nullptr)) { throw init_error(__FILE__, __LINE__); }
	...
	void gfx_engine::set_prim(prim primitive) {
		m_prim = primitive;
	}
	// --==<core_methods>==--
	...
	{
		glDrawArrays(m_prim, buffer->get_offset() / buffer->get_stride(), buffer->get_count());
	}
	void gfx_engine::draw_vtx(v1u buffer_id)
	{
		if (gfx_buf_vtx* vbuf = this->get_cmp_ref<gfx_buf_vtx>(buffer_id).get_ref<gfx_buf_vtx>()->check_cast<gfx_buf_vtx>()) {
			glDrawArrays(m_prim, vbuf->get_offset() / vbuf->get_stride(), vbuf->get_count());
		}
	}
