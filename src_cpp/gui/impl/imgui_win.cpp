    ...
    static void Win32UpdateMousePos()
    {
        ...
        if (HWND focused_hwnd = GetForegroundWindow()) {
            ...
            if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
                ...
                if (GUI::FindViewportByPlatformHandle((void*)focused_hwnd) != NULL)
        ...
        if (HWND hovered_hwnd = WindowFromPoint(mouse_screen_pos)) {
            if (ImGuiViewport* viewport = GUI::FindViewportByPlatformHandle(static_cast<void*>(hovered_hwnd))) {
...
#undef MAP_ANALOG
    ...
    static BOOL CALLBACK Win32UpdateMonitorsEnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
    {
        ...
        ImGuiPlatformIO& io = GUI::GetPlatformIO();
...
// ps: we treat DBLCLK messages as regular mouse down messages, so this code will work on windows classes that have the CS_DBLCLKS flag set. our own example app code doesn't set this flag.
    NWG_API LRESULT Win32MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
...
// helper structure we store in the void* RenderUserData field of each ImGuiViewport to easily retrieve our backend data.
    ...
    static void Win32CreateWindow(ImGuiViewport* viewport)
    {
        ...
        if (viewport->ParentViewportId != 0) {
            if (ImGuiViewport* parent_viewport = GUI::FindViewportByID(viewport->ParentViewportId)) {
    ...
    {
        if (Win32MsgProc(pWindow, unMsg, wParam, lParam)) { return true; }

        if (ImGuiViewport* viewport = GUI::FindViewportByPlatformHandle((void*)pWindow)) {
            switch (unMsg) {
