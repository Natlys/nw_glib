...
namespace NW
{
	...
	txr_smp::txr_smp(gfx_engine& graphics, filter filter_mode, wrap wrap_mode, v4f border_color) :
		txr_smp(graphics)
	{
	...
	// --setters
	void txr_smp::set_slot(v1ui slot) {
		m_slot = slot;
	}
	// --==<core_methods>==--
	bit txr_smp::remake(filter filter_mode, wrap wrap_mode, v4f border_color)
	{
...
namespace NW
{
	...
	txr_smp::txr_smp(gfx_engine& graphics, texture_filters filter, texture_wraps wrap_mode, v4f border_color) :
		txr_smp(graphics)
	{
	...
	// --==<core_methods>==--
	bit txr_smp::remake(texture_filters filter_mode, texture_wraps wrap_mode, v4f border_color)
	{
		if (filter_mode != TXFL_DEFAULT) { m_filter = convert<texture_filters, filter>(filter_mode); }
		if (wrap_mode != TXW_DEFAULT) { m_wrap = convert<texture_wraps, wrap>(wrap_mode); }
		if (wrap_mode == TXW_BORDER) { m_border_color = border_color; }
