...
namespace NW
{
	...
	// --==<core_methods>==--
	bit shd_pxl::remake(cstr source_code)
	{
		...
		if (!gfx_get_err_log(SHD_PIXEL, m_handle)) { return false; }
...
namespace NW
{
	...
	bit shd_pxl::remake(cstr source_code)
	{
		...
		if (m_src_code == "default") { return false; }
