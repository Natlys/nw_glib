...
namespace NWG
{
	...
	frame_buf::~frame_buf() { if (m_native != 0) { glDeleteFramebuffers(1, &m_native); m_native = 0; } }
	// --setters
	...
	void frame_buf::on_draw() {
		glBindFramebuffer(GL_FRAMEBUFFER, m_native);
		glViewport(0, 0, get_size_x(), get_size_y());
	}
	void frame_buf::remake(const frame_buf_info& info)
	{
		if (m_native != 0) { glDeleteFramebuffers(1, &m_native); m_native = 0; }
		if (get_size_x() < 1 || get_size_y() < 1) { return; }

		glGenFramebuffers(1, &m_native);
		glBindFramebuffer(GL_FRAMEBUFFER, m_native);
		...
		for (si32 txi = 0; txi < m_atchs.size(); txi++) {
			...
			glBindTexture(GL_TEXTURE_2D, *static_cast<GLuint*>(atch.get_native()));
			si32 unAttachType = 0;
				...
				txr_info.pxl_format== PXF_R8G8B8A8_SINT32 || txr_info.pxl_format== PXF_R8G8B8A8_UINT32)
			{
				...
				m_info.color_ids.push_back(*static_cast<GLuint*>(atch.get_native()));
			}
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, unAttachType,
				convert_enum<texture_types, si32>(atch.get_txr_type()), *static_cast<GLuint*>(atch.get_native()), 0);
		}
