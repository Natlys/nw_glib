...
namespace GUI
{
    ...
    void    Dx11InvalidateDeviceObjects()
    {
        ...
        if (s_pFontTextureView) { s_pFontTextureView->Release(); s_pFontTextureView = NULL; ImGui::GetIO().Fonts->TexID = NULL; } // We copied s_pFontTextureView to io.Fonts->TexID so let's clear that as well.
        if (s_pIdxBuf) { s_pIdxBuf->Release(); s_pIdxBuf = NULL; }
        if (s_pVtxBuf) { s_pVtxBuf->Release(); s_pVtxBuf = NULL; }
    ...
    static void Dx11ShutdownPlatformInterface() { ImGui::DestroyPlatformWindows(); }
}
#endif  // NWG_GAPI